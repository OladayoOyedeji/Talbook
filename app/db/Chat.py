...
def insert_message(chat_id: int, sender_id: int, content: str, is_read: bool = False):
    message_sql = '''
    INSERT INTO Message (chat_id, sender_id, content, is_read)
    VALUES (%s, %s, %s, %s)
    '''
    mysql_util.execute_sql(message_sql, (chat_id, sender_id, content, is_read), commit=True)
...
def insert():
    ...
    insert_message(chat1, elif_cato_id, "Hi, I love the harp! Is it still available?", is_read=True)
    insert_message(chat1, bigbrovc_id, "Yes! Still available. Do you have any questions?", is_read=True)
    insert_message(chat1, elif_cato_id, "Would you take $150 for it?", is_read=False)
